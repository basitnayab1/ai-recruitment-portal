  ...
  return (
    <div className={`group flex flex-col p-6 ${SURFACE_CARD_INTERACTIVE}`}>
      <div className="flex items-start justify-between gap-3">
        <span className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-gradient-to-br from-violet-500 to-indigo-600 shadow-lg shadow-violet-500/25 transition-transform group-hover:scale-105">
        ...
        <div className="flex flex-wrap justify-end gap-1.5">
          ...
          <span className="rounded-full bg-zinc-100 px-2.5 py-0.5 text-[10px] font-bold text-zinc-600 uppercase dark:bg-zinc-800 dark:text-zinc-400">
      ...
      <Link
        href={`/jobs/${job.id}`}
        className="mt-4 text-lg font-bold text-zinc-900 transition-colors group-hover:text-violet-600 dark:text-zinc-50 dark:group-hover:text-violet-400"
      >
        {job.title}
      </Link>
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        <span className="inline-flex items-center gap-1 rounded-lg bg-zinc-100 px-2.5 py-1 text-xs text-zinc-600 dark:bg-zinc-800 dark:text-zinc-400">
